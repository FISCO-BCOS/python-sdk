...
validcmds = get_validcmds() + RPCConsole.get_all_cmd() + Precompile.get_all_cmd()
    ...
    try:
        ...
        if cmd == "txinput":
            ...
            if os.path.isfile(abi_path) is False:
                raise BcosException(
                    "execute {} failed for {} doesn't exist".format(cmd, abi_path)
                )
            try:
                ...
                raise BcosException("execute {} failed for reason: {}".format(cmd, e))
