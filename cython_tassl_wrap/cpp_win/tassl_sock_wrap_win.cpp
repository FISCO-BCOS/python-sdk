...
//sockerr by 	sockerr = WSAGetLastError ();
char * get_socket_last_error_msg(int  sockerr,char *msgbuf,int bufsize)
{
...
int TasslSockWrap::init(const char *ca_crt_file_,
...
int TasslSockWrap::create_socket()
{
	...
	{
		get_socket_last_error_msg(WSAGetLastError(),msgbuf,sizeof(msgbuf));
		altprint (echo_mode,"create SOCKET error. %s\n",msgbuf);
		return -200;
	}
	unsigned long ulEnable = 1;   
	 //设置成非阻塞模式；
	retval = ioctlsocket(sock, FIONBIO, &ulEnable);
	
	if(retval == SO_ERROR )
	{
		altprint(echo_mode,"create_socket error : ioctlsocket");
		close_socket();
		return -201;
	}
	return 0;
...
int TasslSockWrap::select_socket(int sock_fd,int type, int timeout_msec)//超时时间:毫秒
{
	...
	if(retval > 0)
	{
		...
		if(FD_ISSET(sock_fd,&fdset))
		{
		   ...
		   retval = getsockopt(sock_fd, SOL_SOCKET, SO_ERROR, (char *)&sockerr, &len) ;
		   if(retval< 0)
		   {
				  //获取信息失败
				 select_result = -1003;
...
int TasslSockWrap::connect_socket_async(){
	...
	altprint(echo_mode,"sock = %d,read y to connect [%s:%d]\n",sock,host,port);
	/* 准备通信地址和端口号 */
	...
	altprint(echo_mode,"[in cpp wrap -->] socket connect %d,retval=%d\n",sock,retval);
	int sock_err = 0;
	if(retval < 0 )          //errno == EINPROGRESS表示正在建立链接
	{
		sock_err = WSAGetLastError();
		get_socket_last_error_msg(sock_err,msgbuf,sizeof(msgbuf));
		altprint(echo_mode,"socket connet result ,error %d:%s\n",sock_err,msgbuf);
		if (sock_err == WSAEWOULDBLOCK)
		{
			altprint(echo_mode,"[in cpp wrap -->]connect WSAEWOULDBLOCK %d\n",sock);
			connect_result = select_socket(sock,1,3000);
		}else{
