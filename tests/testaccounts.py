...
def test_p12():
    ...
    with open(p12filename,"rb") as f:
        ...
        p12buff = f.read()
        pwd = b"123456"
        (key, cert, additional_certificates) = pkcs12.load_key_and_certificates(bytes(p12buff),password=pwd,backend=default_backend())
        print("p12 privkey :", key)
        print("p12 privkey size:",key.key_size)
        print("p12 public  bytes:", key.public_key)

        #用crypto加载p12文件，会有warning "PKCS#12 support in pyOpenSSL is deprecated. You should use the APIs in cryptography."
